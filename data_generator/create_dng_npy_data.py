...
def hdr_preprocess_gamma(im_path, use_factorised_data, factor_coeff, reshape=False):
    rgb_img = hdr_image_util.read_hdr_image(im_path)
    hdr_image_util.print_image_details(rgb_img, "before")
...
def apply_preprocess_for_hdr(im_path, args):
    ...
    display_tensor(gray_im_log, True)
    return rgb_img, gray_im_log
...
if __name__ == '__main__':
    ...
    im_path = os.path.join("/Users/yaelvinker/PycharmProjects/lab/data_generator/hard_images/synagogue.hdr")
    rgb_img, gray_im = apply_preprocess_for_hdr(im_path, args)
    data = {'input_image': gray_im, 'display_image': rgb_img}
    output_path = os.path.join('/Users/yaelvinker/PycharmProjects/lab/data_generator/temp_f/a.npy')
    np.save(output_path, data)
    print(output_path)



    im_path = os.path.join("/Users/yaelvinker/PycharmProjects/lab/data_generator/merged_0006_20160726_113607_220.npy")
    data = np.load(im_path, allow_pickle=True)
    input_im = data[()]["input_image"]
    color_im = data[()]["display_image"]
    hdr_image_util.print_tensor_details(input_im, "input_im ")
    display_tensor(input_im, True)
    hdr_image_util.print_tensor_details(color_im, "display_image ")
    display_tensor(color_im, False)
    #
    # parser = argparse.ArgumentParser(description="Parser for gan network")
